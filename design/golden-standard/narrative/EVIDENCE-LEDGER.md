...
The vertical slice proves Dagg's method, product logic and control model. It is not yet evidence of customer outcomes, production scale or systems Dagg operates.

## Status system

| Code | Meaning | Public use |
|---|---|---|
| P | Public now | Dagg-owned definition, offer, method or design principle. No customer result implied. |
| R | Representative / illustrative | Constructed product artifact with synthetic data and a visible provenance label. |
| V | Verification required | Technical, legal or commercial fact requiring internal documentation. |
| C | Customer approval required | Customer name, data, workflow, image, quote, result or identifiable circumstance. |
| X | Not permitted now | Unsupported proof of managed operations, live production, scale or results. |

## Claim ledger

`P` in this ledger means the company owner has approved the statement as Dagg's current positioning, method, offer or architecture intent. It is not an external assurance. Technical, security and customer-outcome statements still require the narrower evidence specified below.

| Claim | Status | Evidence owner | Authoritative source | Verified date | Boundary |
|---|---|---|---|---|---|
| Dagg helps existing companies become AI-native and new companies begin that way. | P | Christian Perez, CEO | Founder brief accepted in Masterplan Part I | 2026-08-30 | Positioning and offer, not a universal outcome guarantee. |
| Dagg combines strategic foresight, company-building experience and engineering. | P | Christian Perez, CEO | Founder brief accepted in Masterplan Part I | 2026-08-30 | Corporate capability statement. Avoid unsupported superlatives. |
...
| Dagg currently operates customer systems. | X | Christian Perez, CEO | No accepted public evidence | 2026-08-30 | Do not use until verified public evidence exists. |
| `10x`, `100x`, `1000x`, fully autonomous, guaranteed or transform-or-die claims. | X | Christian Perez, CEO | Explicit exclusion in masterplan and P1 | 2026-08-30 | Not part of this narrative. |

## Required labels for constructed artifacts
...
Do not show a node network. Show one readable company-context record that gains depth while its identity stays constant.

The record contains:

- **Evidence:** approved source, captured statement, system touched, provenance.
- **Operating reality:** work item, owner, handoff, exception, unresolved decision.
- **Decision state:** selected outcome, reason, boundary, next accountable owner.

`Map → Decide → Build → Govern → Operate` changes how the same context is used. It does not replace the context with five unrelated screens.

### Factory

Pass the same record into one two-mode build instrument:

- **Automate what stays:** retained system, approved tool connection, required context, proposed agent action, evaluation, human approval, disable/rollback.
- **Rebuild what should change:** target workflow contract, application boundary, data/interface requirements, software behavior, acceptance tests, accountable release owner, migration and way back.
...
| Section | CEO / board | COO / sponsor | CTO / CIO | CISO / risk |
|---|---|---|---|---|
| Hero | Partner or automation shop? | Do they understand operating reality? | More than a model wrapper? | Can AI act without control? |
| Stakes | Real strategic change or AI hype? | Where does handoff loss occur? | Why do generic tools not solve it? | Does speed outrank governance? |
| Decision | Will Dagg always recommend a build? | Does the method remove work before adding a layer? | When integrate, automate or rebuild? | Where are veto and risk decisions? |
| WorkGraph | What durable company asset remains? | Are ownership and exceptions visible? | Structured context or process picture? | Which sources, roles and data are permitted? |
| Factory | When does strategy become real? | What is built and who owns it? | How are tools, evaluations and rollback handled? | How are actions bounded and approved? |
| Operating record | Who remains accountable? | What happens at a material exception? | Can we inspect, debug and reverse? | What data and action were allowed, and who decided? |
...
- Factory lacks a concrete bill of materials;
- the operating record lacks a stop, human decision, owner and way back;
- a CEO must understand graph architecture before understanding Dagg;
- `Systems operated` appears as current evidence or category.
