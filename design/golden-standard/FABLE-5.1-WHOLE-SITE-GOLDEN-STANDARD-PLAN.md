...
**FS5 reconciliation status.** This file preserves the failure-first review of
the FS4B baseline. Where its pre-reconciliation wording conflicts with the
current `FS5-DRAFT-ROUTE-SPECIFICITY-AND-EXECUTION-PROOF.md`, the reconciled
FS5 package, content contract, founder reset, route page maps, evidence ledger,
asset registry and slot manifests are binding. Snapshot `6ef754faf00f…` is a
review baseline only and is forbidden as FS5 evidence identity.
...
**How we get there.** Five steps, two non-overlapping Claude lanes, one assembler, no shared files (§8). Codex closes ten decisions and amends the authorities; the two lanes write six routes to one section-by-section specification (§9); Codex assembles once, freezes one snapshot and runs a fail-closed evidence rig (§10); Fable accepts independently; publication work is separated (§11).

**Activation and gates (§12).** The ten open items are resolved defaults, R1–R10, owned and recorded by Codex. FS5 is activated for two bounded, non-overlapping Claude lanes as soon as Codex records R1–R10 in the FS5 text and reconciles the authority files (content contract, Home founder reset, page maps, evidence ledger and image manifests) to §9. Founder gates are only a materially different design-world direction and final launch, publish or canonical promotion; everything else is Codex operating acceptance. No publish, deploy or canonical promotion happens without Christian's explicit launch approval.
...
| Executive accessibility | Home ≤520 default words, first section 35–55 words, zero unexplained Dagg vocabulary above the fold | 9 | 9 |
...
| Density calibration | 9 | 7 | 5 | 6 | 5 | 8 | 9 | Route bands (Home 420–520, routes 260–600); progressive copy capped; artifact replaces prose |
...
| 2 | FS5 writing | Claude Code, two bounded non-overlapping lanes | days 1–3 | Source edits in owned files only; one implementation report per lane; assembler check mode only | Both lanes' failure-first reports; protected hashes unchanged |
...
Claude lane A may edit only:
`preview/golden-standard/home/src/{body.html,direction.css,direction.js}` · `routes/transformation/src/{body.html,direction.css,direction.js}` · `routes/company/src/{body.html,direction.css}` · `evidence/FULL-SITE-STAGING/FS5-LANE-A-IMPLEMENTATION-REPORT.md`.

Claude lane B may edit only:
`preview/golden-standard/routes/workgraph/src/{body.html,direction.css,direction.js}` · `routes/build/src/{body.html,direction.css,direction.js}` · `routes/trust/src/{body.html,direction.css,direction.js}` · `evidence/FULL-SITE-STAGING/FS5-LANE-B-IMPLEMENTATION-REPORT.md`.
...
- [ ] Image lane: Transformation 4:5 desktop-side use of the mobile asset if R3 = A; Company uses registered C-DF-SEAM-01 at intrinsic 5:2 in the 640×256 desktop/tablet and 350×140 mobile carrier if R5 = A; register in `ASSET-REGISTRY.json` and slot manifests with hashes.
...
- [x] Capture the protected FS4 Home first viewport losslessly at 390×844,
  1024×768, 1440×900 and 1536×1024 from immutable snapshot
  `6ef754faf00f234ba503ddbc83e8226759054a4b57929b9757561b3a6fdfdc50`.
  Baseline authority:
  `evidence/FULL-SITE-STAGING/FS4B/home-lossless-baseline-6ef754faf00f/manifest.json`.
...
- [ ] Edit only the files in §8.2; keep every existing `data-*` hook. Every
  public text unit implements the exact `data-copy-ref`, `data-copy-kind` and
  `data-copy-scope` entry in `narrative/COPY-UNIT-MANIFEST.json`; uncovered or
  nested public copy references fail closed.
...
- [ ] Confirm protected Home hero copy, selected asset hashes and rendered
  first-viewport region against
  `evidence/FULL-SITE-STAGING/FS5-PROTECTED-INVARIANT-BASELINE.json`. Whole
  route-file hashes are not valid invariants because FS5 intentionally edits
  those files.
...
- [ ] Apply `authority/FS5-VISUAL-PAIR-REVIEW-PROTOCOL.md` in full: canonical
  breakpoint pairs, randomized grayscale silhouettes, asset fidelity,
  whitespace/rhythm and caption-free two-second causal recognition.
...
Word budget: today ≈450. Moving progressive record rows plus the second lifecycle paragraph under native disclosure must bring default-visible copy to ≤430 without shortening accepted sentences.
...
| Section | Copy | Image | Component · interaction | Input | RM / no-JS | Credibility | Owner | Acceptance |
|---|---|---|---|---|---|---|---|---|
| 1 Position hero (recomposed) | Contract Build 1 verbatim; page disclosure moved above the stage; **Factory stage rows:** `Illustrative Factory build plan · not a client deployment` · `Approved intervention — Prepare the payment draft; preserve human approval for release.` · `Specification and permission — Use approved context from WG-034. Prepare only. Never release.` · `Evaluation — Withhold when source records disagree.` · `Review state — Draft preparation passed evaluation; payment release remains prohibited.` · `Evidence return — Evidence returns to WG-034; the Finance owner retains release authority.` | Operational Evidence as one coded five-row stage; B-OE-MS-01 is retired and must not load | 5/7 Paper composition: proposition left; coded stage right; coral rule on Approved intervention, sage rule on Review state; static | none | Identical; identical | Disclosure above the stage; provenance row | `body.html`, `direction.css` | First-viewport contract at three widths; disclosure precedes stage; stage static; no bitmap |
...
Word budget: 360–535 default-visible words. The widened ceiling accommodates the coded first-screen proof replacing the bitmap; it does not authorize additional prose.

### 9.5 Trust (`trust/src/body.html`, `direction.css`, `direction.js`)
...
| Section | Copy | Image | Component · interaction | Input | RM / no-JS | Credibility | Owner | Acceptance |
|---|---|---|---|---|---|---|---|---|
| 1 Position opening | Contract Company 1 verbatim plus three recalled commitments: `Decision` — `Name what changes, what deliberately does not, who owns the outcome and what evidence would change the recommendation.` · `Build` — `Name the required context, tools, permissions, evaluations, approval and way back before release.` · `Escalation` — `Return material exceptions to an accountable owner.` | C-DF-SEAM-01, independently authored and rendered at intrinsic 5:2; v2 pair is review-only | Copy-led proposition, quiet seam, then three ruled Paper rows; no cards, icons or dominant hero | none | Still; same | none (P method statements) | `body.html`, `direction.css` | First two accountability rows are fully inside the first viewport at 1440×900 and 1024×768. At 390×844 the chain and at least the first 44 px of its Decision row begin in view; the rest continues in DOM order. The seam remains 640×256 at 1440, uses the responsive seven-column 5:2 carrier at 1024 and is 350×140 at 390. If the seam fails the two-second gate, stop and return R5 to Codex. |
...
| 4 Partnership standard | Contract 4 verbatim; the progressive disclosure keeps all five commitments and the boundary. Decision, Build and Escalation deliberately recall the opening chain; `Five commitments · diligence boundary` remains exact. | none | Ink 5/7; `<details>` with `align-self:start`; closed row 64 px with its rule on the summary | Details | Native | Boundary `…no residency, retention, encryption or isolation assurance is implied here.` | `direction.css`, `body.html` | Closed details height 64 ± 4 px at 1440; link `Inspect the permission model` → `/routes/trust/#permission` |
| 5 Accountability and close | `Accountability has names.` · `Corporate direction and technical standards have named owners. No customer name, workflow, quote or result is published without permission, provenance and a stated boundary.` · `Direction — Christian Pérez, Chief Executive Officer` · `Technical standards — Simon Lundmark, Chief Technology Officer`; then the accepted close | none | Leadership rows low on Company and separate from the opening chain; close with primary and mailto | Buttons | Identical | Titles V before publication | `body.html` | Titles verified before publication; mailto subject `Dagg partnership` |

### 9.7 Assessment (bounded state repair and hooks) and Impact (unchanged; robots meta in FS6)
...
| Assessment 5 Inquiry | Contract 5 verbatim incl. consent, validation messages, submitting, duplicate, success and error copy | none | Form hidden until JS; manifest-backed state on button and live region; 450 ms preview reveal; control stays enabled; local in-flight guard prevents a second timer/request; `<noscript>` mail path | Enter/click submit; first invalid field focused; second submit exposes duplicate state | RM: 0 ms reveal; no-JS: form hidden, noscript visible | `This local preview has not sent any data…` | Codex | All five `aria-invalid`; exact button/status state pairs; zero outbound requests |
...
| First-viewport contracts | Typed DOM geometry of the §9 nodes at 1440×900, 1024×768, 390×844 | Required copy, actions and labels are fully inside. A `START44` carrier is horizontally contained, begins inside the viewport and exposes at least 44 px. Multi-node contracts state `ALL_FULL(n)` or `ANY_FULL`; no universal whole-hero proxy is permitted. | `route-matrix.json` |
| Type and layout | computed font sizes, gutters, field width, splits and text-range line boxes at each viewport vs §5.1 | within ±0.5 px; integer H1 line ceilings at 1440 / 1024 / 390 are Home 2/2/4, Transformation 3/4/4, WorkGraph 2/2/5, Build 4/4/4, Trust 2/2/3, Company 3/3/5, Assessment 3/3/3 and Impact 2/2/3. Missing geometry fails closed. | `route-matrix.json` |
...
**Verdict.** FS5 as originally drafted was not sufficient; FS5 as reconciled in the current FS5 package is. Activation condition: Codex records R1–R10 and reconciles the content contract, founder reset, route maps, evidence ledger and image manifests. When that is done and a same-session Fable read emits the exact activation sentence, FS5 is active for two bounded, non-overlapping Claude lanes with the current package file ownership. Publication, deployment and canonical promotion remain forbidden without Christian's explicit launch approval.
...
| R5 | Company opening | A: copy-led proposition, registered 5:2 seam in a capped 640×256 carrier, then three accountability rows · B: keep hero, fix details and label only | A |
| R6 | Home default-word ceiling | A: enforce ≤500 and name the trim · B: accept contract ceiling 520 | B for FS5, A revisited in FS6 |
| R7 | WorkGraph disclosure allocation | Keep lifecycle paragraph one default-visible; move lifecycle paragraph two and the four lifecycle descriptions under `Why the record persists`. | Approved |
...
Sequence: Codex records R1–R10 → Codex reconciles the contract, reset, maps, ledger and image manifests → Fable reconciliation read (one pass) → FS5 active for two bounded non-overlapping Claude lanes → Codex assembles once and runs §10 evidence → final Fable read → FS6 → Christian's launch gate. Nothing before the launch gate publishes, deploys or promotes canonical routes.
...
## Appendix A — Pre-reconciliation FS4B divergences (historical; non-binding for FS5 activation)

This table records the rejected input snapshot only. Current activation
authority is §9, §12 and the reconciled authority files.
