/* Direction B1 — finite product-mechanism enhancement. Static HTML remains
 * complete with scripts blocked; enhanced controls are created only when the
 * corresponding state model is available. */
(function () {
  "use strict";
  ...
  var FACTORY_MODELS = {
    ...
    automate: {
      messages: ["Requirement linked to WG-034.", "Human approval boundary retained.", "Approved context attached.", "Agent and tool boundary assembled.", "Mismatch hold verified.", "Ready for governed review."],
      bodies: ["Resolve before release.", "Prepare; preserve approval for release.", "Link approved records.", "Bind agent and permission.", "Withhold mismatched release.", "Return evidence."]
    },
    rebuild: {
  ...
  function initGovernedException() {
    ...
    var panels = Array.prototype.slice.call(scope.querySelectorAll("[data-b-exception-panel]"));
    if (!host || panels.length !== 2) return;

    var labels = { bounded: "Within bounds", exception: "Governed exception" };
    var buttons = panels.map(function (panel, index) {
      ...
      if (!panel.id) panel.id = "b1-boundary-" + key;
      button.setAttribute("aria-controls", panel.id);
      button.appendChild(document.createTextNode(labels[key] || key));
    ...
    function select(key) {
      buttons.forEach(function (button) {
        button.setAttribute("aria-pressed", button.getAttribute("data-b-exception-mode") === key ? "true" : "false");
      });
